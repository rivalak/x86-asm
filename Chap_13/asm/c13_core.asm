         ...
         mem_0_4_gb_seg_sel    equ  0x08       ; 整个 0-4GB 内存的段的选择子
         ...
         core_entry       dd start             ; 核心代码段入口点 #10 指示段内偏移
                          dw core_code_seg_sel ; 指定内存代码段的选择子
...
allocate_memory:                               ; 分配内存
         ...
         and ebx,0xfffffffc
         add ebx,4                             ; 强制对齐
         test eax,0x00000003                   ; 下次分配的起始地址最好是 4 字节对齐
         ...
         ; -------------------------------------------------------------------------------
set_up_gdt_descriptor:                         ; 在 GDT 内安装一个新的描述符
                                               ; 输入：EDX:EAX = 描述符
                                               ; 输出：CX = 描述符的选择子
          push eax
         push ebx
         ...
         mov ebx,core_data_seg_sel             ; 切换到核心数据段
          ...
          ; 将 GDT 寄存器的基地址和边界信息保存到 pgdt 这个内存位置
...
load_relocate_program:                         ; 加载并重定位用户程序
         ...
         call sys_routine_seg_sel:read_hard_disk_0
          ...
          mov ebx,eax
         and ebx,0xfffffe00                    ; 使之 512 字节对齐（能被 512 整除的数，
         add ebx,512                           ; 低 9 位都为 0
         test eax,0x000001ff                   ; 程序的大小正好是 512 的倍数吗？
  ...
  .b1:
         ...
         mov ecx,0x00c09600                    ; 4KB 粒度的堆栈段描述符
          call sys_routine_seg_sel:make_seg_descriptor
         call sys_routine_seg_sel:set_up_gdt_descriptor
  ...
  .b3:
         ...
         repe cmpsd                            ; 每次比较 4 字节
         jnz .b4
         mov eax,[esi]                         ; 若匹配，esi 恰好指向其后的地址数据
  ...
  .b4:
         ...
         ; -------------------------------------------------------------------------------
start:
         mov ecx,core_data_seg_sel             ; 使 ds 指向核心数据段
         mov ds,ecx
...
return_point:                                  ; 用户程序返回点
          mov eax,core_data_seg_sel            ; 使 ds 指向核心数据段
          mov ds,eax
